...
| 4 | 同步图像 / 视频最新模型与价格 | ✅ | GPT Image 2.5 + Gemini Flash Lite Image；Imagen/旧 Veo 下架 |
| 5 | 提交部署 | 🟡 | |

**今日小结**：对话与图像/视频模型库已对齐官网；完成价目核验并更新用户公告（仅列新增模型）。

**明日优先**
- [ ] 若有 DB 价格覆盖，后台重新导入 `yuhao-model-pricing-all.csv`
- [ ] 登录后确认公告弹窗与新图像模型可选
- [ ] 下一项：模型广场与公开状态页
